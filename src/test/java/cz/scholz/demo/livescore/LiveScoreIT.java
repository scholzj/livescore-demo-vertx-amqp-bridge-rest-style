...
public class LiveScoreIT {
    ...
    @Test
    public void testInvalidRequest(TestContext context)
    {
        final Async asyncInvalidRequest = context.async();
                    ...
                    if (openRes.succeeded()) {
                        ...
                        ProtonReceiver recv = conn.createReceiver(responseAddress).handler((delivery, msg) -> {
                            ...
                            if (body instanceof AmqpValue) {
                                context.assertEquals(405, msg.getApplicationProperties().getValue().get("status"));
                                asyncInvalidRequest.complete();
                            }
                        }).open();
        ...
        asyncInvalidRequest.awaitSuccess(10000);
    }
                    ...
                    if (openRes.succeeded()) {
                        ...
                        conn.createSender("/scores").openHandler(openResult -> {
                            if (openResult.succeeded()) {
                                ...
                                Message messageGetScore = message();
                                messageGetScore.setReplyTo(responseAddress);
                                Map props = new HashMap<Object, Object>();
                                props.put("method", "GET");
                                messageGetScore.setApplicationProperties(new ApplicationProperties(props));

                                sender.send(messageGetScore);
                            }
                        }).open();
                    }
                    else
