...
public class LiveScore extends AbstractVerticle {
    ...
    public void start(Future<Void> fut) {
        ...
        bridge.start(amqpHostname, amqpPort, res -> {
            if (res.succeeded())
            {
                ...
                // /scores entrypoint
                bridge.createConsumer("/scores").setMaxBufferedMessages(100).handler(this::scores);
    ...
    public void scores(Message<Object> msg)
    {
        JsonObject msgBody = (JsonObject)msg.body();

        if ("POST".equals(msgBody.getJsonObject("application_properties").getString("method")))
        {
            addGame(msg);
        }
        else if ("PUT".equals(msgBody.getJsonObject("application_properties").getString("method")))
        {
            setScore(msg);
        }
        else if ("GET".equals(msgBody.getJsonObject("application_properties").getString("method")))
        {
            getScores(msg);
        }
        else
        {
            if(msg.replyAddress() != null) {
                ...
                response.put("application_properties", new JsonObject().put("status", 405));
                response.put("body", new JsonObject().put("error", "Method not allowed"));
                msg.reply(response);
            }
        }
    }
